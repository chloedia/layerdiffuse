...
class LayerDiffusionForForge(scripts.Script):
    ...
    def ui(self, *args, **kwargs):
        with gr.Accordion(open=False, label=self.title()):
            ...
            fg_additional_prompt = gr.Textbox(
                placeholder="Additional prompt for foreground.",
                visible=False,
                label="Foreground Additional Prompt",
            )
            bg_additional_prompt = gr.Textbox(
                placeholder="Additional prompt for background.",
                visible=False,
                label="Background Additional Prompt",
            )
            blend_additional_prompt = gr.Textbox(
                placeholder="Additional prompt for blended image.",
                visible=False,
    ...
    def process_before_every_sampling(
        ...
        fg_image = (
            crop_and_resize_image(rgba2rgbfp32(fg_image), resize_mode, height, width)
            if fg_image is not None
            else None
        )
        bg_image = (
            crop_and_resize_image(rgba2rgbfp32(bg_image), resize_mode, height, width)
            if bg_image is not None
            else None
        )
        blend_image = (
            crop_and_resize_image(rgba2rgbfp32(blend_image), resize_mode, height, width)
        ...
        fg_additional_prompt = (
            fg_additional_prompt + ", " + original_prompt
            if fg_additional_prompt != ""
            else None
        )
        bg_additional_prompt = (
            bg_additional_prompt + ", " + original_prompt
            if bg_additional_prompt != ""
            else None
        )
        blend_additional_prompt = (
            blend_additional_prompt + ", " + original_prompt
            if blend_additional_prompt != ""
            else None
        )
